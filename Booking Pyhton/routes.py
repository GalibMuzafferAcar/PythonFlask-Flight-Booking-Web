...
def booking_page():
    ...
    if current_user.is_active == 1:
        ...
        return render_template("logged-in-booking-page.html",tolocation=tolocation,fromlocation=fromlocation,tl=tl,fl=fl)
    else:
        if from_location and tolocation:
            print('yey')
    ...
    if departure_date == '':
        if sort=='sorted':
            ...
            query = db.select([Flight,Flight_Details]).where(
                db.and_(Flight.from_location == from_location, Flight.to_location == to_location,Flight.flight_id==Flight_Details.flight_id))
            result = connection.execute(query)
            ucuslar = result.fetchall()
    else:
        if sort=='sorted':
...
def general_flights(from_location,to_location,departure_date,adult,child,sort):
    ...
    print(ucret)
    veli.id=ucret
    return render_template("general-flights.html",ucuslar=ucuslar,ucuslarl=ucuslarl,yolcu=yolcu,ucret=ucret,detay=detay)
check=0
...
def basket():
    ...
    if delete!=None:
        ...
        result=connection.execute(query)
    if alldelete!=None:
        query = db.delete(Ticket_Info).where(db.and_(Ticket_Info.user_id==current_user.id,Ticket_Info.status=='pending'))
        result = connection.execute(query)
    print(edit)
    print(delete)
    print(alldelete)
    query = db.select([Ticket_Info]).where(db.and_(Ticket_Info.user_id==current_user.id,Ticket_Info.status=='pending'))
    result=connection.execute(query)
    ...
    for i in range(fidsl):
        query=db.select([Flight,Flight_Details]).where(db.and_(Flight.flight_id==fids[i][2],Flight.flight_id==Flight_Details.flight_id))
        result=connection.execute(query)
    ...
    if check == 'puansız':
        ...
        result = connection.execute(query)
        return redirect(url_for('basket'))
    return render_template("basket.html",fids=fids,fidsl=fidsl,biletler=biletler,totalfiyat=totalfiyat,puan=puan,offprice=offprice,msg=msg)
...
def edit_ticket():
    ...
    query=db.select([Flight,Flight_Details]).where(db.and_(Flight.flight_id==ticket[0][2],Flight.flight_id==Flight_Details.flight_id))
    result=connection.execute(query)
...
def account():
    ...
    puan=result.fetchall()
    print(puan)
    return render_template("account.html", name=name, id=id, aktif=aktif, aktifl=aktifl, pasif=pasif, pasifl=pasifl,puan=puan)
...
def dashboard():
    ...
    if airline_Name and from_location and to_location and departure_time and depart_date and arrival_time and total_seats and status:
        ...
        query = db.select([Flight,Flight_Details]).where(db.and_(Flight.flight_id == flightid,Flight.flight_id==Flight_Details.flight_id))
        result = connection.execute(query)
        flightdetay = result.fetchall()
        flightdetayl = len(flightdetay)
        print(flightdetay)
